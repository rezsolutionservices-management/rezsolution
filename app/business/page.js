...
export default function Business() {
  ...
  const inputStyle = { width: "100%", backgroundColor: "#1C1C1C", border: "1px solid #444444", borderRadius: "4px", padding: "0.75rem 1rem", color: "#FFFFFF", fontFamily: "Barlow", fontSize: "1rem", outline: "none", marginTop: "0.4rem" };
  const labelStyle = { color: "#CCCCCC", fontFamily: "Barlow", fontWeight: 500, fontSize: "0.9rem", display: "block" };
  ...
  return (
    <main>
      <section style={{ backgroundColor: "#1C1C1C", padding: "4rem 2rem", textAlign: "center", borderBottom: "4px solid #F5C000" }}>
        <p style={{ color: "#F5C000", fontFamily: "Barlow Condensed", fontWeight: 700, fontSize: "1rem", letterSpacing: "3px", textTransform: "uppercase", marginBottom: "1rem" }}>Partner With Us</p>
        <h1 style={{ fontFamily: "Barlow Condensed", fontWeight: 800, fontSize: "clamp(2.5rem, 6vw, 4rem)", color: "#FFFFFF", lineHeight: 1.1, marginBottom: "1rem" }}>Business Accounts</h1>
      ...
      <section style={{ backgroundColor: "#111111", padding: "0 2rem", borderBottom: "1px solid #333333" }}>
        <div style={{ maxWidth: "900px", margin: "0 auto", display: "flex" }}>
          {[{ id: "business", label: "Business Account" }, { id: "eatery", label: "CMO Eatery Partner" }].map(tab => (
            <button key={tab.id} onClick={() => setActiveTab(tab.id)} style={{ padding: "1.25rem 2rem", fontFamily: "Barlow Condensed", fontWeight: 700, fontSize: "1.1rem", letterSpacing: "1px", border: "none", borderBottom: activeTab === tab.id ? "3px solid #F5C000" : "3px solid transparent", backgroundColor: "transparent", color: activeTab === tab.id ? "#F5C000" : "#AAAAAA", cursor: "pointer" }}>
              {tab.label}
            </button>
          ))}
        </div>
      ...
      <section style={{ backgroundColor: "#111111", padding: "5rem 2rem", borderBottom: "4px solid #F5C000" }}>
        <div style={{ maxWidth: "900px", margin: "0 auto" }}>

          {activeTab === "business" && (
            <div style={{ display: "flex", gap: "4rem", flexWrap: "wrap" }}>
              <div style={{ flex: "1", minWidth: "240px" }}>
                <h2 style={{ fontFamily: "Barlow Condensed", fontWeight: 800, fontSize: "2rem", color: "#FFFFFF", marginBottom: "1rem" }}>Business Account Program</h2>
                <p style={{ color: "#CCCCCC", fontSize: "0.95rem", lineHeight: 1.7, marginBottom: "1.5rem" }}>
                  Regular delivery needs? Set up a business account for streamlined ordering, flexible billing, and priority service.
                </p>
                {["Priority scheduling", "Flexible weekly or monthly billing", "Direct line to your driver", "Consistent, reliable service", "Standard rate applies"].map(item => (
                  <div key={item} style={{ display: "flex", gap: "0.5rem", marginBottom: "0.6rem" }}>
                    ...
                    <span style={{ color: "#CCCCCC", fontSize: "0.95rem" }}>{item}</span>
                  </div>
                ))}
              </div>
              <div style={{ flex: "2", minWidth: "280px" }}>
                {bizSubmitted ? (
                  <div style={{ backgroundColor: "#1a3a1a", border: "2px solid #2E7D32", borderRadius: "8px", padding: "2.5rem", textAlign: "center" }}>
                    <h3 style={{ fontFamily: "Barlow Condensed", fontWeight: 700, fontSize: "1.5rem", color: "#FFFFFF", marginBottom: "0.5rem" }}>Application Received!</h3>
                    <p style={{ color: "#CCCCCC" }}>Thanks {bizForm.name} — we will be in touch shortly to set up your account.</p>
                  </div>
                ) : (
                  <form onSubmit={handleBizSubmit} style={{ display: "flex", flexDirection: "column", gap: "1.25rem" }}>
                    <h3 style={{ fontFamily: "Barlow Condensed", fontWeight: 700, fontSize: "1.5rem", color: "#F5C000" }}>Sign Up</h3>
                    ...
                    <button type="submit" style={{ backgroundColor: "#F5C000", color: "#1C1C1C", padding: "0.9rem 2rem", borderRadius: "4px", fontFamily: "Barlow Condensed", fontWeight: 700, fontSize: "1.1rem", letterSpacing: "1px", border: "none", cursor: "pointer", alignSelf: "flex-start" }}>SUBMIT APPLICATION</button>
                  </form>
                )}
              </div>
            </div>
          ...
          {activeTab === "eatery" && (
            <div style={{ display: "flex", gap: "4rem", flexWrap: "wrap" }}>
              <div style={{ flex: "1", minWidth: "240px" }}>
                <h2 style={{ fontFamily: "Barlow Condensed", fontWeight: 800, fontSize: "2rem", color: "#FFFFFF", marginBottom: "1rem" }}>CMO Eatery Partner Program</h2>
                <p style={{ color: "#CCCCCC", fontSize: "0.95rem", lineHeight: 1.7, marginBottom: "1.5rem" }}>
                  A dedicated delivery tier for food businesses operating on CMO territory. Cafes, home kitchens, catering, band kitchens, and food trucks welcome.
                </p>
                ...
                <div style={{ backgroundColor: "#1C1C1C", border: "1px solid #F5C000", borderRadius: "6px", padding: "1rem", marginTop: "1.5rem" }}>
                  <p style={{ color: "#F5C000", fontSize: "0.85rem", fontFamily: "Barlow Condensed", fontWeight: 700 }}>NOTE ON RATES</p>
                  <p style={{ color: "#CCCCCC", fontSize: "0.85rem", marginTop: "0.25rem" }}>Eatery partner delivery rates are negotiated per partner and not publicly published.</p>
                </div>
              </div>
              <div style={{ flex: "2", minWidth: "280px" }}>
                {eaterySubmitted ? (
                  <div style={{ backgroundColor: "#1a3a1a", border: "2px solid #2E7D32", borderRadius: "8px", padding: "2.5rem", textAlign: "center" }}>
                    <h3 style={{ fontFamily: "Barlow Condensed", fontWeight: 700, fontSize: "1.5rem", color: "#FFFFFF", marginBottom: "0.5rem" }}>Application Received!</h3>
                    <p style={{ color: "#CCCCCC" }}>Thanks {eateryForm.name} — we will be in touch to discuss your delivery partnership.</p>
                  </div>
                ) : (
                  <form onSubmit={handleEaterySubmit} style={{ display: "flex", flexDirection: "column", gap: "1.25rem" }}>
                    <h3 style={{ fontFamily: "Barlow Condensed", fontWeight: 700, fontSize: "1.5rem", color: "#F5C000" }}>Eatery Partner Sign Up</h3>
